...
# The files will be created within the Docker container
RESULTS_SYS_SINGLE_FILEPATH="$SYSBENCH_SINGLE_DIR/final_result.txt"
RESULTS_SYS_MULTI_FILEPATH="$SYSBENCH_MULTI_DIR/final_result.txt"
...
BIND_RESULTS_SYS_SINGLE_TEMPLATE="
      - type: bind
        source: $RESULTS_SYS_SINGLE_FILEPATH
        target: /home/mysql/benchmarks/sysbench_single/final_result.txt"

BIND_RESULTS_SYS_MULTI_TEMPLATE="
      - type: bind
        source: $RESULTS_SYS_MULTI_FILEPATH
        target: /home/mysql/benchmarks/sysbench_multi/final_result.txt"

BIND_RESULTS_DBT2_SINGLE_TEMPLATE="
      - type: bind
        source: $RESULTS_DBT2_SINGLE_FILEPATH
        target: /home/mysql/benchmarks/dbt2_single/final_result.txt"

BIND_RESULTS_DBT2_MULTI_TEMPLATE="
      - type: bind
        source: $RESULTS_DBT2_MULTI_FILEPATH
        target: /home/mysql/benchmarks/dbt2_multi/final_result.txt"
    ...
    for CONTAINER_NUM in $(seq $NUM_API_NODES); do
      ...
      deploy:
        ...
        if [ "$NUM_MYSQL_NODES" -gt 0 ]; then
            ...
            template+="$BIND_RESULTS_SYS_SINGLE_TEMPLATE"
            template+="$BIND_RESULTS_DBT2_SINGLE_TEMPLATE"

            if [ "$NUM_MYSQL_NODES" -gt 1 ]; then
                template+="$BIND_AUTOBENCH_SYS_MULTI_TEMPLATE"
                template+="$BIND_AUTOBENCH_DBT2_MULTI_TEMPLATE"

                template+="$BIND_RESULTS_SYS_MULTI_TEMPLATE"
                template+="$BIND_RESULTS_DBT2_MULTI_TEMPLATE"
            fi
        fi
