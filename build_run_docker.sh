...
if [ $USER_ID != 1000 ]; then
    OS=$(uname -s)
    if [ "$OS" != "Darwin" ]; then
        if [ "$VOLUME_TYPE" == "local" -o "$VOLUME_TYPE_BENCH_DIRS" == "local" ]; then
            echo "WARNING: The user id is $USER_ID and not 1000, which is the UID inside the container." >&2
            echo "         We're using OS $OS (not Mac). We might therefore have difficulties writing to local directories." >&2
            echo "         It is recommended to place everything in volumes using flags --volumes-in-local-dir --bench-dirs-in-volumes." >&2
        fi
    fi
fi
...
mkdir -p $SYSBENCH_SINGLE_DIR $SYSBENCH_MULTI_DIR $DBT2_SINGLE_DIR $DBT2_MULTI_DIR
...
BIND_DBT2_SINGLE_CONF_TEMPLATE="$(printf "$VOLUME_TEMPLATE" "$DBT2_CONF_SINGLE_FILEPATH" "$BENCH_DIR/dbt2_single/dbt2_run_1.conf")"
BIND_DBT2_MULTI_CONF_TEMPLATE="$(printf "$VOLUME_TEMPLATE" "$DBT2_CONF_MULTI_FILEPATH" "$BENCH_DIR/dbt2_multi/dbt2_run_1.conf")"

BIND_AUTOBENCH_SYS_SINGLE_TEMPLATE="$(printf "$VOLUME_TEMPLATE" "$AUTOBENCH_SYS_SINGLE_FILEPATH" "$BENCH_DIR/sysbench_single/autobench.conf")"
BIND_AUTOBENCH_SYS_MULTI_TEMPLATE="$(printf "$VOLUME_TEMPLATE" "$AUTOBENCH_SYS_MULTI_FILEPATH" "$BENCH_DIR/sysbench_multi/autobench.conf")"
BIND_AUTOBENCH_DBT2_SINGLE_TEMPLATE="$(printf "$VOLUME_TEMPLATE" "$AUTOBENCH_DBT2_SINGLE_FILEPATH" "$BENCH_DIR/dbt2_single/autobench.conf")"
BIND_AUTOBENCH_DBT2_MULTI_TEMPLATE="$(printf "$VOLUME_TEMPLATE" "$AUTOBENCH_DBT2_MULTI_FILEPATH" "$BENCH_DIR/dbt2_multi/autobench.conf")"
...
for CONTAINER_NUM in $(seq $NUM_MGM_NODES); do
    ...
    # We add volumes to the data dir for debugging purposes
    add_volume_to_template "dataDir_$SERVICE_NAME" "$DATA_DIR/mgmd" $VOLUME_TYPE no
    add_volume_to_template "logDir_$SERVICE_NAME" "$DATA_DIR/log" $VOLUME_TYPE no
...
for CONTAINER_NUM in $(seq $NUM_DATA_NODES); do
    ...
    # We add volumes to the data dir for debugging purposes
    add_volume_to_template "dataDir_$SERVICE_NAME" "$DATA_DIR/ndb_data" $VOLUME_TYPE yes
    add_volume_to_template "logDir_$SERVICE_NAME" "$DATA_DIR/log" $VOLUME_TYPE no
...
if [ $NUM_MYSQL_NODES -gt 0 ]; then
    for CONTAINER_NUM in $(seq $NUM_MYSQL_NODES); do
      ...
      deploy:
        ...
        # We add volumes to the data dir for debugging purposes
        add_volume_to_template "dataDir_$SERVICE_NAME" "$DATA_DIR/mysql" $VOLUME_TYPE no
        # This is for debugging
        add_volume_to_template "mysqlFilesDir_$SERVICE_NAME" "$DATA_DIR/mysql-files" $VOLUME_TYPE no
...
if [ $NUM_API_NODES -gt 0 ]; then
    for CONTAINER_NUM in $(seq $NUM_API_NODES); do
      ...
      deploy:
        ...
        template+="$VOLUMES_FIELD"
        if [ "$NUM_MYSQL_NODES" -gt 0 ]; then
            add_volume_to_template "sysbench_single" "$BENCH_DIR/sysbench_single" $VOLUME_TYPE_BENCH_DIRS no
            add_volume_to_template "dbt2_single" "$BENCH_DIR/dbt2_single" $VOLUME_TYPE_BENCH_DIRS no
            if [ "$NUM_MYSQL_NODES" -gt 1 ]; then
                add_volume_to_template "sysbench_multi" "$BENCH_DIR/sysbench_multi" $VOLUME_TYPE_BENCH_DIRS no
                add_volume_to_template "dbt2_multi" "$BENCH_DIR/dbt2_multi" $VOLUME_TYPE_BENCH_DIRS no
            fi
        fi
